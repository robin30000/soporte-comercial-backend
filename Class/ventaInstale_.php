...
class ventaInstale
{
        ...
        try {
            ...
            if ($hoy >= $hora_minima && $hoy <= $hora_maxima) {
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    $stmt = $this->_DB->SeguimientoPedidos()->prepare("INSERT INTO ventasInstaleTiendas (fecha_atencion,jornada_atencion, pedido,documento_cliente,numero_contacto_cliente,login_despacho,observacion_canal,regional,documento_tecnico,nombre_tecnico,categoria, hora_ingreso, fecha_ingreso)
        ...
        try {
            ...
            $stmt = $this->_DB->SeguimientoPedidos()->prepare("SELECT
                                                ...
                                                FROM
                                                    ventasInstaleTiendas
                                                WHERE 1 = 1 $condicion
                                                    AND login_despacho = :login_despacho LIMIT $offset, $pagesize");
            $stmt->execute(array(':login_despacho' => $data['login']));
    ...
    public function observaciones()
    {
        try {
            $hora = date('Y-m-d');
            $stmt = $this->_DB->SeguimientoPedidos()->prepare("SELECT * FROM observacion_venta_instale_despacho WHERE hora BETWEEN '$hora 00:00:00' and '$hora 23:59:59'");
            $stmt->execute();
            if ($stmt->rowCount() > 0) {
